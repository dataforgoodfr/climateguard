...
import pandas as pd
from app.mediatree_utils import get_new_plaintext_from_whisper, get_video_urls, get_url_mediatree, get_response_single_export_api, get_auth_token, fetch_video_url, download_media
...
def test_get_video_urls(monkeypatch):
    ...
    pd.testing.assert_series_equal(videos_urls["media_url"].reset_index(drop=True),
...
def mock_get_auth_token(mocker):
    ...
    return mocker.patch('mediatree_utils.get_auth_token', return_value="mocked_token")
...
def mock_get_response_single_export_api_mp3(mocker):
    ...
    return mocker.patch('mediatree_utils.get_response_single_export_api', return_value=mock_response_json)
...
def mock_fetch_video_url(mocker):
    # Mock the `fetch_video_url` function to return a mock URL.
    return mocker.patch('mediatree_utils.fetch_video_url', return_value='https://example.com/test.mp3')

@pytest.fixture
def mock_download_media_mp4(mocker):
    # Mock `requests.get` to simulate an MP3 file response (or any other file type).
    mock_response = mocker.MagicMock()
    with open('tests/label_misinformation/data/test.mp4', 'rb') as f:
        mock_response.content = f.read()  # Read the binary content of a test file.
    mock_response.status_code = 200
    return mocker.patch('mediatree_utils.download_media', return_value=mock_response)
...
def test_get_new_plaintext_from_whisper_mp3(mocker, mock_get_auth_token, mock_get_response_single_export_api_mp3, mock_fetch_video_url, mock_download_media_mp3):
    mock_get_auth_token = mocker.patch('mediatree_utils.get_auth_token', return_value="mocked_token")
...
def test_get_new_plaintext_from_whisper_mp4(mock_get_auth_token, mock_get_response_single_export_api_mp4, mock_fetch_video_url, mock_download_media_mp3):
