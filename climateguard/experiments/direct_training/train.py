...
def get_data():
    ...
    dataset = dataset.filter(lambda example: isinstance(example["misinformation"], bool))
    dataset = dataset.filter(lambda example: not isinstance(example["misinformation"], str))
    dataset = dataset.map(
        lambda example: {
...
text: {transcript}"""
    ...
    dataset = get_data()
    logger.info(dataset)
    logger.info(dataset["test"].to_pandas().value.unique())
    dataset = dataset.map(
        lambda example: {
            "messages": [
    ...
    if args.wandb:
        run = wandb.init(
            ...
            name=f"gmguarino/climateguard-{args.checkpoint.split('/')[1]}-classification-sft"
    ...
    model_merged.push_to_hub(
        f"gmguarino/climateguard-{args.checkpoint.split('/')[1]}-classification-sft"
    )
