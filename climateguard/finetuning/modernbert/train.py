...
def sample_dataset(
    ...
    for record in dataset:
        if record["misinformation"] and n_true < n_max:
            records.append(
                {
                    "text": record["plaintext_whisper"]
                    .lower()
                    .replace(".", "")
                    .replace(",", ""),
                    "label": 1,
            ...
            n_true += 1
        elif not record["misinformation"] and n_false < n_max:
            records.append(
                {
                    "text": record["plaintext_whisper"]
                    .lower()
        ...
        if record["misinformation_claims"] and n_true < n_max:
            for claim in record["misinformation_claims"]:
                for label in claim["labels"]:
                    ...
                    start_idx = max(start_claim - args.chunk_size // 3, 0)
                    end_idx = start_idx + len(claim_text) + args.chunk_size
